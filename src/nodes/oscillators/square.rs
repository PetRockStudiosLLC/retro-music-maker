...
impl Node for SquareOscillator {
    ...
    fn name(&self) -> &str { "SquareOscillator" }
    ...
    fn inputs(&self) -> Vec<PortInfo> {
        vec![
            ...
            PortInfo { name: "pulse_width".to_string(), port_type: PortType::Control },
        ]
    }
    ...
    fn default_params(&self) -> NodeParams {
        ...
        params.insert("pulse_width".to_string(), ParamValue::Float(self.pulse_width));
        params
    }
    ...
    fn set_param(&mut self, name: &str, value: ParamValue) {
        match name {
            ...
            "pulse_width" => { if let ParamValue::Float(f) = value { self.pulse_width = f.max(0.0).min(1.0); } }
            _ => {}
        }
    }
    ...
    fn process(&mut self, block_size: BlockSize, _input: Option<&[f32]>) -> AudioBuffer {
        ...
        for _ in 0..block_size {
            let sample = if self.phase < self.pulse_width { 1.0 } else { -1.0 };
