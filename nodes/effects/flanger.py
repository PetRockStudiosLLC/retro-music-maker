...
class Flanger(Node):
    ...
    def _setup_params(self):
        self._params["rate"] = 0.5
        self._params["depth"] = 0.002
        self._params["feedback"] = 0.5
    ...
    def process(self, block_size: int) -> np.ndarray:
        ...
        rate = self._params.get("rate", 0.5)
        depth = self._params.get("depth", 0.002)
        feedback = self._params.get("feedback", 0.5)
        ...
        for i in range(block_size):
            ...
            flanged = buffer[int(buf_idx)]
            output[i] = audio_in[i] + flanged
            buffer[int(phase) % len(buffer)] = (
                audio_in[i] + flanged * feedback
            )
            phase += 1.0
